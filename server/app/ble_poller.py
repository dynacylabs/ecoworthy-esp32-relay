...
TARGET_ADDRESS = int(TARGET_BLE_MAC.replace(":", ""), 16)
...
class BLEPoller:
        ...
        try:
            try:
                await asyncio.wait_for(
                    connected_event.wait(), timeout=BLE_CONNECT_TIMEOUT_SECONDS + 5,
                )
            except asyncio.TimeoutError:
            ...
            if not state["connected"]:
                logger.warning("BLE connect to %s failed", TARGET_BLE_MAC)
                return
            ...
            if not notify_targets:
                logger.warning("No notify/indicate characteristics found on %s", TARGET_BLE_MAC)
                return

            pool = await get_pool()
            device_id = await get_or_create_device(pool, TARGET_BLE_MAC)
            ...
            def make_notify_handler(char_uuid: str):
                def on_notify(handle: int, data: bytearray):
                    self._last_activity = time.monotonic()
                    asyncio.create_task(self._store_reading(pool, device_id, char_uuid, bytes(data)))
                return on_notify
            ...
            try:
                while True:
                    try:
                        ...
                        pass
                    if time.monotonic() - self._last_activity > BLE_STALL_SECONDS:
                        logger.warning("No data for %.0fs, reconnecting", BLE_STALL_SECONDS)
                        return
            finally:
    ...
    async def _store_reading(self, pool, device_id: int, characteristic: str, data: bytes):
